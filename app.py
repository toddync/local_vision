...
class AIChatApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Assistente de Chat com IA")
        ...
        self.announce(
            "Aplicação Assistente de Chat com IA iniciada. Pronto para conversar."
        )
    ...
    def create_header(self):
        ...
        title_label = tk.Label(
            header_frame,
            text="Assistente de Chat com IA",
    ...
    def create_chat_area(self):
        ...
        self.chat_frame = tk.Frame(
            chat_container, bg=self.colors["white"], relief="solid", bd=1
        )
        ...
        self.chat_canvas.aria_label = "Mensagens do chat"
    ...
    def create_input_area(self):
        ...
        self.attach_btn.aria_label = "Anexar imagem à mensagem"
        ...
        self.clear_btn.aria_label = "Limpar todas as mensagens do chat"

        # Main input area
        input_frame = tk.Frame(
            input_container, bg=self.colors["white"], relief="solid", bd=2
        )
        ...
        self.user_input.aria_label = "Digite sua mensagem aqui"
        ...
        self.send_button.aria_label = "Enviar mensagem"
        ...
        shortcuts_label = tk.Label(
            input_container,
            text="Atalhos de teclado: Enter para enviar, Shift+Enter para nova linha",
            font=("Segoe UI", 9),
            bg=self.colors["background"],
    ...
    def create_status_bar(self):
        self.status_frame = tk.Frame(
            self.root, bg=self.colors["white"], height=30, relief="solid", bd=1
        )
    ...
    def setup_keyboard_navigation(self):
        ...
        widgets = [self.attach_btn, self.user_input, self.send_button, self.clear_btn]
    ...
    def announce(self, message):
        ...
        print(f"Leitor de tela: {message}")
    ...
    def append_message(self, sender, message, is_image=False):
        ...
        bubble_frame = tk.Frame(
            message_frame, bg=bg_color, relief="solid", bd=1, padx=15, pady=10
        )
        ...
        if sender == "user":
            self.announce(
                f"Você disse: {message if not is_image else 'Imagem anexada'}"
            )
        else:
            self.announce(f"Assistente: {message}")

    def show_typing_indicator(self):
        self.is_typing = True
        self.typing_indicator.config(text="IA está digitando...")
        self.announce("IA está digitando uma resposta")
    ...
    def send_message(self):
        ...
        if not user_text and not self.image_to_send:
            messagebox.showinfo(
                "Sem Conteúdo",
                "Por favor, digite uma mensagem ou anexe uma imagem antes de enviar.",
            )
            return
        ...
        if self.image_to_send:
            self.append_message("user", "[Imagem Anexada]", is_image=True)

        # Clear input
        self.user_input.delete("1.0", tk.END)
        self.update_status("Enviando mensagem...")
    ...
    def clear_chat(self):
        if not self.conversation_history:
            messagebox.showinfo("Chat Já Vazio", "Não há mensagens para limpar.")
            return

        result = messagebox.askyesno(
            "Limpar Chat",
            "Tem certeza de que deseja limpar todas as mensagens do chat?",
        )
        if result:
            for widget in self.scrollable_frame.winfo_children():
                widget.destroy()
            self.conversation_history.clear()
            self.update_status("Chat limpo")
            self.announce("Histórico do chat limpo")
    ...
    def attach_image(self):
        ...
        if file_path:
            try:
                ...
                self.update_status(f"Imagem '{filename}' anexada")
                self.announce(f"Imagem {filename} anexada com sucesso")
                messagebox.showinfo(
                    "Imagem Anexada",
                    f"Imagem '{filename}' anexada com sucesso!\n\nEnvie sua mensagem para analisar a imagem.",
                )
            except Exception as e:
                messagebox.showerror(
                    "Erro", f"Não foi possível ler o arquivo de imagem: {e}"
                )
                self.image_to_send = None
    ...
    async def get_bot_response(self, user_message, image_data=None):
        ...
        try:
            if image_data:
                self.update_status("Analisando imagem...")
                description = await self._get_image_description(image_data)
                self.hide_typing_indicator()
                self.append_message("assistant", f"Eu vejo uma imagem: {description}")
                self.conversation_history.append(
                    {
                        "role": "assistant",
                        "content": f"Eu vejo uma imagem: {description}",
                    }
                )
                ...
                self.update_status("Gerando resposta...")
                response_text = await self._generate_text_response(
                    user_message, self.conversation_history
                )
                self.hide_typing_indicator()
    ...
    async def _generate_text_response(self, prompt: str, history: list) -> str:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                return f"Não foi possível gerar resposta: {str(e)}"
...
if __name__ == "__main__":
    ...
    root.iconname("Assistente de Chat com IA")
